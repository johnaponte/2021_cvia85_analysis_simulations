...
d_d0_a <-power %>%
  ...
  ggplot() +
  aes(x = assay_OR, y = power, linetype = as.factor(day_corr), color = as.factor(day_OR)) +
  geom_line()+
  facet_wrap(~nsubjects) +
  scale_x_continuous("Assay OR") +
  scale_y_continuous("Empirical power") +
...
d_a0_d <-power %>%
  ...
  ggplot() +
  aes(x = day_OR, y = power, linetype = as.factor(day_corr), color = as.factor(assay_OR)) +
  geom_line()+
  facet_wrap(~nsubjects) +
...
pdf(file = "reports/cvia_085_power_analysis.pdf", width = 16, height = 11)
plots
wplots
dev.off()
